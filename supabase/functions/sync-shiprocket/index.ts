...
async function syncShipments(
  ...
  const from = repair ? new Date(Date.now() - 30 * 86_400_000).toISOString().slice(0, 10) : after.slice(0, 10);
  ...
  while (hasMore) {
    ...
    for (const s of shipments) {
      try {
        ...
        }, { onConflict: "awb_code", ignoreDuplicates: false });
        if (error) throw error;
        counters.records_updated++;
        if (s.updated_at > watermarkTo) watermarkTo = s.updated_at;
      } catch (err) {
        counters.records_failed++;
        await recordSyncError(db, runId, job.integration_key, job.entity_type, s.awb_code || String(s.id), "MAPPING_ERROR", err instanceof Error ? err.message : String(err), s);
      }
    }
