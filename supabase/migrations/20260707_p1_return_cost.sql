-- =============================================================================
-- P1 HIGH-02: Calculate return_cost_inr from RTO shipment freight
-- Previously hardcoded to 0 in get_profitability_kpis.
-- Fix: sum freight_total_inr from shipments in RTO status for the period,
-- using channel_created_at for date scoping (consistent with operations KPIs).
-- return_cost_inr is a memo cost; contribution_margin and net_profit are
-- unchanged until the waterfall UI gains a dedicated return-cost row.
-- =============================================================================
...
-- HIGH-02: actual return freight from RTO shipments in the period
rto_freight AS (
  SELECT COALESCE(SUM(s.freight_total_inr), 0) AS total
  FROM shipments s
...
SELECT json_build_object(
  ...
  'return_cost_inr',          ROUND(rf.total, 2),
...
FROM rev_cogs rc, ship_totals st, ad a, booked b, rto_freight rf, exp_opex eo, exp_marketing em, exp_capex ec;
$function$;
