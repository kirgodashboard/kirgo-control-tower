-- Fix orphan_shipments trust check: exclude CUSTOM-channel shipments (expected by design)
-- and set threshold of 20 for historical WooCommerce-channel gaps from April 2025.
-- CUSTOM channel = manually created in Shiprocket; no WC order exists by design.
...
BEGIN
  ...
  -- Exclude CUSTOM-channel shipments (created manually in Shiprocket; no WC order by design).
  -- Threshold 20 covers 17 pre-existing April-2025 historical gaps; alert if new ones appear.
  SELECT COUNT(*) INTO v_orphans
  FROM shipments
  WHERE order_id IS NULL AND COALESCE(channel, '') != 'CUSTOM';
  checks := checks || jsonb_build_object('key','orphan_shipments','label','Unexpected orphaned shipments',
    'status', CASE WHEN v_orphans <= 20 THEN 'GREEN' ELSE 'AMBER' END,
    'expected','<= 20', 'actual', v_orphans,
    'detail','Non-CUSTOM shipments with no linked order (CUSTOM channel excluded — expected manual shipments)');
  ...
  RETURN jsonb_build_object(
    'trust_score', score, 'status', final_status,
