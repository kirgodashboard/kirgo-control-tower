...
-- Version: v2.2  |  Date: 2026-06-17
-- Source: DATABASE_SCHEMA.md v2.2 · DATA_DICTIONARY.md v2.1 · BUSINESS_RULES.md v2.0
--
...
CREATE TABLE gateway_settlements (
  ...
  CONSTRAINT gateway_settlements_gateway_chk   CHECK (gateway IN ('easebuzz','infibeam','shiprocket_cod','gokwik','razorpay')),
...
-- =============================================================================
-- DOMAIN 8: IMPORT TRACKING  (v2.2)
-- Created after users (both tables reference users.id).
-- import_errors references import_runs, so import_runs must come first.
-- Both tables are admin-only. The import pipeline writes via the service role key
-- (which bypasses RLS); admin users can read the audit trail via the dashboard.
-- =============================================================================
...
CREATE TABLE import_errors (
  ...
  -- resolved_by and resolved_at must both be set when status is resolved/ignored
...
COMMENT ON COLUMN import_errors.error_code IS 'Machine-readable code. 24 codes defined in IMPORT_STATUS_TRACKING.md §Error Code Reference. '
                                              'Examples: DUPLICATE_AWB, UNRESOLVED_SKU, ORPHAN_SHIPMENT, BALANCE_BREAK, DATE_SEQ_ERROR.';
COMMENT ON COLUMN import_errors.severity IS 'error = row excluded from DB | warning = row imported with flag | info = expected skip (duplicate).';
COMMENT ON COLUMN import_errors.source_row_snapshot IS 'Full JSON copy of the source row. Enables re-import after the root cause is fixed without returning to the source file.';
COMMENT ON COLUMN import_errors.resolution_status IS 'unresolved = needs attention | resolved = fixed and re-imported | ignored = known acceptable | deferred = next release.';
...
-- import_runs indexes
CREATE INDEX import_runs_source_idx         ON import_runs (source);
CREATE INDEX import_runs_status_idx         ON import_runs (status);
CREATE INDEX import_runs_started_at_idx     ON import_runs (run_started_at DESC);
CREATE INDEX import_runs_recon_status_idx   ON import_runs (reconciliation_status);
CREATE INDEX import_runs_triggered_by_idx   ON import_runs (triggered_by);

-- import_errors indexes
CREATE INDEX import_errors_run_id_idx       ON import_errors (import_run_id);
...
CREATE INDEX import_errors_resolution_idx   ON import_errors (resolution_status);
CREATE INDEX import_errors_unresolved_idx   ON import_errors (import_run_id, error_code)
...
-- ---------------------------------------------------------------------------
-- import_runs — admin-only (pipeline writes via service role key which bypasses RLS)
-- ---------------------------------------------------------------------------
CREATE POLICY "import_runs_select_admin"  ON import_runs FOR SELECT USING  (public.current_app_role() = 'admin');
CREATE POLICY "import_runs_insert_admin"  ON import_runs FOR INSERT WITH CHECK (public.current_app_role() = 'admin');
CREATE POLICY "import_runs_update_admin"  ON import_runs FOR UPDATE USING  (public.current_app_role() = 'admin') WITH CHECK (public.current_app_role() = 'admin');
CREATE POLICY "import_runs_delete_admin"  ON import_runs FOR DELETE USING  (public.current_app_role() = 'admin');

-- ---------------------------------------------------------------------------
-- import_errors — admin-only (resolution workflow is admin-gated)
-- ---------------------------------------------------------------------------
CREATE POLICY "import_errors_select_admin"  ON import_errors FOR SELECT USING  (public.current_app_role() = 'admin');
CREATE POLICY "import_errors_insert_admin"  ON import_errors FOR INSERT WITH CHECK (public.current_app_role() = 'admin');
CREATE POLICY "import_errors_update_admin"  ON import_errors FOR UPDATE USING  (public.current_app_role() = 'admin') WITH CHECK (public.current_app_role() = 'admin');
CREATE POLICY "import_errors_delete_admin"  ON import_errors FOR DELETE USING  (public.current_app_role() = 'admin');
